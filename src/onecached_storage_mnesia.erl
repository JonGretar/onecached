...
-module(onecached_storage_mnesia).
...
store_item(_, #storage_command{key=Key} = Command) when is_list(Key) ->
    store_item(mnesia, Command#storage_command{key=list_to_binary(Key)});
store_item(_, #storage_command{key=Key, flags=Flags, exptime=Exptime, data=Data})
...
store_item(_, #storage_command{exptime=Exptime} = StorageCommand) ->
...
% {error, Reason}
get_item(_, Key) when is_list(Key) ->
    get_item(mnesia, list_to_binary(Key));
get_item(_, Key) ->
    mnesia_get(Key, value, dirty).

delete_item(_, Key) when is_list(Key) ->
...
update_item_value(_, Key, Value, Operation) when is_list(Key) ->
    update_item_value(mnesia, list_to_binary(Key), Value, Operation);
update_item_value(_, Key, Value, Operation) ->
    F = fun() ->
